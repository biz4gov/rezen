...
import type { ItemFaturado, ProductData } from '../types';
import { Package, Trash2, PlusCircle } from 'lucide-react';

interface ItensFaturadosProps {
  items: ItemFaturado[];
  setItems: (items: ItemFaturado[]) => void;
  products: ProductData[];
}

const ItensFaturados: React.FC<ItensFaturadosProps> = ({ items, setItems, products }) => {

  const handleItemChange = (index: number, field: keyof ItemFaturado, value: string) => {
  ...
  const removeItem = (index: number) => {
    if (window.confirm('Tem certeza de que deseja excluir este item faturado?')) {
    ...
    <div className="space-y-6">
      ...
      {items.length === 0 && (
        <div className="text-center py-10 border-2 border-dashed border-gray-300 rounded-lg">
          ...
          <p className="mt-2 text-gray-500">Nenhum item encontrado. Analise uma Nota Fiscal ou adicione manualmente.</p>
        </div>
      )}
          ...
          <div key={index} className="bg-gray-50 p-4 rounded-lg border border-gray-200 relative">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-8 gap-4">
                <div className="sm:col-span-2">
                    ...
                    <input type="text" value={item.item_codigo} onChange={(e) => handleItemChange(index, 'item_codigo', e.target.value)} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm sm:text-sm" />
                </div>
                <div className="sm:col-span-3">
                    <label className="block text-sm font-medium text-gray-700">Descrição</label>
                    <textarea value={item.item_descricao} onChange={(e) => handleItemChange(index, 'item_descricao', e.target.value)} rows={2} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm sm:text-sm" />
                </div>
                <div className="sm:col-span-3">
