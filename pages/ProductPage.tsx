...
import { Upload, X, Image as ImageIcon, Loader2, FileText, Save, ArrowLeft } from 'lucide-react';
...
const ProductPage: React.FC<ProductPageProps> = ({ onSave, onCancel, initialData }) => {
    const getInitialState = (): ProductData => {
    ...
    const [product, setProduct] = useState<ProductData>(getInitialState());
    ...
    const handlePdfUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const base64 = await fileToBase64(file);
        setProduct(prev => ({ ...prev, produto_folheto_pdf: { name: file.name, base64 } }));
        e.target.value = ''; // Reset file input
    };

    const removePdf = () => setProduct(prev => ({ ...prev, produto_folheto_pdf: null }));
    const removeImage = (index: number) => setProduct(prev => ({ ...prev, produto_imagens: prev.produto_imagens.filter((_, i) => i !== index) }));

    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        setIsSaving(true);
        await onSave(product);
        // Navigation will be handled by the parent, no need to set isSaving to false
    };
                    ...
                    <div className="container mx-auto flex justify-between items-center">
                        <div className="flex items-center gap-4">
                            <button type="button" onClick={onCancel} className="p-2 rounded-full hover:bg-gray-200"><ArrowLeft size={20} /></button>
                             <h1 className="text-2xl font-bold text-gray-800">{initialData ? 'Editar Produto' : 'Cadastrar Novo Produto'}</h1>
                        </div>
                        <button type="submit" disabled={isSaving || !product.produto_nome} className="inline-flex items-center justify-center rounded-md border border-transparent bg-blue-800 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-blue-900 disabled:bg-blue-400 disabled:cursor-not-allowed">
                ...
                <div className="flex-grow overflow-y-auto">
                    <div className="container mx-auto p-8">
                         <div className="max-w-4xl mx-auto space-y-8">
                            ...
                            <div className="bg-white p-6 rounded-lg shadow-sm border">
                                <h2 className="text-xl font-semibold text-gray-800 mb-4">Mídia</h2>
                                            ...
                                            <div className="mt-2 grid grid-cols-3 sm:grid-cols-5 gap-4">
                                                {product.produto_imagens.map((image, index) => (
                                                    <div key={index} className="relative group aspect-square">
                                                        ...
                                                        <button type="button" onClick={() => removeImage(index)} className="absolute top-1 right-1 bg-black bg-opacity-50 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity" aria-label="Remover imagem"><X size={14} /></button>
                                                    </div>
                                                ))}
                                            </div>
                                        )}
                                    ...
                                    <div>
                                        ...
                                        {product.produto_folheto_pdf ? (
                                            <div className="mt-2 flex items-center justify-between bg-gray-100 p-2 rounded-md border">
                                                <div className="flex items-center overflow-hidden"><FileText size={20} className="text-red-500 mr-2 flex-shrink-0" /><span className="text-sm text-gray-700 truncate" title={product.produto_folheto_pdf.name}>{product.produto_folheto_pdf.name}</span></div>
                                                <button type="button" onClick={removePdf} className="p-1 text-gray-500 hover:text-red-600" aria-label="Remover PDF"><X size={16} /></button>
                                            </div>
                                        ) : (
                                            <div className="mt-2 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
                                                <div className="space-y-1 text-center">
